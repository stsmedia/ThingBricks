...
func (t *DataStreamTests) TestCreateDataStream() {
	...
	t.Post(account+"/1"+project+"/1"+datastreamgroup+"/1"+datastream, contentType, bytes.NewBufferString(string(body)))
	t.AssertOk()
}

func (t *DataStreamTests) TestCreateDataStreamInvalidJson() {
	t.Post(account+"/1"+project+"/1"+datastreamgroup+"/1"+datastream, contentType, bytes.NewBufferString("{\"name1\":\"foo\"}"))
...
func (t *DataStreamTests) TestUpdateDataStream() {
	...
	t.Put(account+"/1"+project+"/1"+datastreamgroup+"/1"+datastream+"/1", contentType, bytes.NewBufferString(string(body)))
	t.AssertOk()
...
func (t *DataStreamTests) TestUpdateDataStreamInvalidJson() {
	t.Put(account+"/1"+project+"/1"+datastreamgroup+"/1"+datastream+"/1", contentType, bytes.NewBufferString("{\"id\":1, \"name1\":\"foo\"}"))
	t.AssertStatus(http.StatusNotAcceptable)
	t.AssertContains("Data stream name required")
}

func (t *DataStreamTests) TestDeleteDataStream() {
	t.Delete(account + "/1" + project + "/1" + datastreamgroup + "/1" + datastream + "/1")
	t.AssertOk()
}
